...
class ControllersSupportWelshTranslationISpec
  ...
  "all the views requested via GET" should {
    ...
    object TestPrecondition {
      ...
      val checkYourAnswers: TestPrecondition = () => stubRetrievePartnershipDetails(testJourneyId)(OK, testPartnershipInformationWithCompanyProfile)
      val confirmCompanyName: TestPrecondition = () => stubRetrieveCompanyProfile(testJourneyId)(status = OK, body = Json.toJsObject(testCompanyProfile))
    }

    val allGETUrlsToBeTested: TableFor2[String, TestPrecondition.TestPrecondition] =
      Tables.Table(
        ("urlToBeTested", "doThisToCreateTestPrecondition"),
        (s"$baseUrl/$testJourneyId/company-registration-number", TestPrecondition.noNeeded),
        (s"$baseUrl/$testJourneyId/check-your-answers-business", TestPrecondition.checkYourAnswers),
        (s"$baseUrl/$testJourneyId/sa-utr", TestPrecondition.noNeeded),
        (s"$baseUrl/$testJourneyId/confirm-company-name", TestPrecondition.confirmCompanyName),
        (s"$baseUrl/$testJourneyId/self-assessment-postcode", TestPrecondition.confirmCompanyName)
      )
    ...
    "display welsh translation when cy cookie is specified" in {
      ...
      await(insertJourneyConfig(
        ...
        journeyConfig = testDefaultJourneyConfig
      ))
  ...
  "all the views in case of error (after a POST)" should {
    ...
    val allPOSTUrlsToBeTested: TableFor1[String] =
      Tables.Table(
        "urlToBeTested",
        s"$baseUrl/$testJourneyId/company-registration-number",
        s"$baseUrl/$testJourneyId/sa-utr",
        s"$baseUrl/$testJourneyId/self-assessment-postcode"
      )
